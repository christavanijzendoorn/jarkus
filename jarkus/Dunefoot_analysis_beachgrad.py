# -*- coding: utf-8 -*-
"""
Created on Mon Aug 31 10:35:33 2020
...
var = 'Beach_gradient_der'
pickle_file = DirDimensions + var + '_dataframe.pickle'    
beach_gradient = pickle.load(open(pickle_file, 'rb')) #load pickle of dimension    

var = 'Beach_width_der'
pickle_file = DirDimensions + var + '_dataframe.pickle'    
beach_width = pickle.load(open(pickle_file, 'rb')) #load pickle of dimension    
...
# Plot overviews and trends for Beach gradient
begin_yr = 1980
end_yr = 2017
DF = beach_gradient
variable = 'beach gradient der'
method = ''
bgrad_filtered, bgrad_nourished, bgrad_not_nourished, bgrad_SFnourished = plot_overview_types(DF, filter_file, begin_yr, end_yr, variable, method, DirDFAnalysis)

unit = '-'
ylimit = [-0.08,0]
DF_type = 'filtered' # 'filtered', 'not_nourished'
plot_trend(bgrad_filtered, variable, method, unit, DF_type, begin_yr, end_yr , ylimit)

# Plot overviews and trends for Beach width
begin_yr = 1980
end_yr = 2017
DF = beach_width
variable = 'beach width der'
method = ''
bwidth_filtered, bwidth_nourished, bwidth_not_nourished, bwidth_SFnourished = plot_overview_types(DF, filter_file, begin_yr, end_yr, variable, method, DirDFAnalysis)

unit = '-'
ylimit = [0,300]
DF_type = 'filtered' # 'filtered', 'not_nourished'
plot_trend(bwidth_filtered, variable, method, unit, DF_type, begin_yr, end_yr , ylimit)


#%%
not_nourished_trscts = []
DF_noNans = bgrad_not_nourished.isna().sum()
for i in DF_noNans.index:
    if DF_noNans.loc[i] != len(bgrad_not_nourished.index):
        not_nourished_trscts.append(i)

#%%

def plot_trend_method(DF, variable, unit, begin_yr, end_yr, ylimit):
    ...
    for i, a in enumerate(area_bounds):
        ...
        print('There are ' + n_Dia + 'transects in ' + variable)
               
        if DF_area.empty == False and DF_area.dropna(how='all').empty == False:
    
            mean_per_year_filt, median_per_transect_filt, mean_of_trends_filt_array, trend_of_yearly_mean_filt_array, trend_of_yearly_mean_filt, rsquared_of_yearly_mean_filt, ci95_per_year = get_filtered_trends(DF_area, threshold)
            ...
            label1 = region + ' ('  + str(round(trend_of_yearly_mean_filt,2)) + ' m/m/yr, $r^{2}$ = ' + str(round(rsquared_of_yearly_mean_filt,3)) + ', n = ' + n_Dia + ')'
            
            axs[i].plot(mean_per_year_filt, color = color, label = label1, marker = 'o', markersize = 8)
            axs[i].errorbar(mean_per_year_filt.index, mean_per_year_filt, ci95_per_year, color = color, linestyle = 'dashed', elinewidth = 3, capsize = 8, capthick = 3, label = '95% confidence interval')
            axs[i].plot(mean_per_year_filt.index, trend_of_yearly_mean_filt_array, color = color, linestyle = 'dashed')
...
plot_trend_method(bgrad_filtered, variable, unit, begin_yr, end_yr, ylimit)        
...
plot_trend_method(bwidth_filtered, variable, unit, begin_yr, end_yr, ylimit)     
