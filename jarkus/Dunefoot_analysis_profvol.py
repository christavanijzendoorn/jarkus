...
var = 'Active_profile_volume'
pickle_file = DirDimensions + var + '_dataframe.pickle'    
profile_volume = pickle.load(open(pickle_file, 'rb')) #load pickle of dimension    
...
# Plot overviews and trends for active profile volume
begin_yr = 1965
end_yr = 2017
DF = profile_volume
variable = 'profile volume'
method = ''
profvol_filtered, profvol_nourished, profvol_not_nourished, profvol_SFnourished = plot_overview_types(DF, filter_file, begin_yr, end_yr, variable, method, DirDFAnalysis)

unit = '$m^{3}$'
ylimit = [0,10000]
DF_type = 'filtered' # 'filtered', 'not_nourished'
plot_trend(profvol_filtered, variable, method, unit, DF_type, begin_yr, end_yr , ylimit)

#%%
not_nourished_trscts = []
DF_noNans = profvol_not_nourished.isna().sum()
for i in DF_noNans.index:
    if DF_noNans.loc[i] != len(profvol_not_nourished.index):
...
def plot_trend_method(DF, variable, unit, begin_yr, end_yr, ylimit):
    ...
    for i, a in enumerate(area_bounds):
        ...
        if DF_area.empty == False and DF_area.dropna(how='all').empty == False:
            ...
            label1 = region + ' ('  + str(round(trend_of_yearly_mean_filt,2)) + ' $m^{3}$/yr, $r^{2}$ = ' + str(round(rsquared_of_yearly_mean_filt,3)) + ', n = ' + n_Dia + ')'
...
begin_yr = 1965
end_yr = 2017
variable = 'active profile volume'
unit = '$m^{3}$'
ylimit = [2000, 15000]        
     
plot_trend_method(profvol_filtered, variable, unit, begin_yr, end_yr, ylimit)        
