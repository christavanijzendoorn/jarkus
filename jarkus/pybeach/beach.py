...
class Profile:
    ...
    def __init__(self, x, z, window_size=5):
        ...
        """
        assert isinstance(x, np.ndarray) & (np.ndim(x) == 1), 'x should be of type ndarray and shape (m,).'
        ...
        if np.any(flag > 0):
            ...
            print(f'Input profiles should be oriented from landward (left) to seaward (right), '
                          f'some inputted profiles appear to have the sea on the left. This may cause errors.')
    ...
    def predict_dunetoe_ml(self, clf_name, no_of_output=1, dune_crest='max', **kwargs):
        ...
        **kwargs : arguments
        ...
        dt_probabilities = ds.interp_to_grid(self.x_interp,
                                             self.x,
                                             dt_probabilities)
        ...
        dt_index = np.array(
            [np.flip(
                np.argsort(row[crest:])[-no_of_output:], 0
    ...
    def predict_dunetoe_mc(self, dune_crest='max', shoreline=True, hanning_window=None, **kwargs):
        ...
        **kwargs : arguments
        ...
        dt_index = np.array([np.argmax(
            np.gradient(
                np.gradient(
                    np.convolve(row[ind_dunecrest:ind_mwl], window, 'same'),
                    0.5),
                0.5)) + ind_dunecrest for (row, ind_dunecrest, ind_mwl) in zip(self.z, dune_crest_loc, shoreline_loc)])
    ...
    def predict_dunetoe_pd(self, dune_crest='max', shoreline=True, **kwargs):
        ...
        **kwargs : arguments
        ...
        for j, row in enumerate(self.z):
            p1 = np.array([self.x[dune_crest_loc[j]],
                           row[dune_crest_loc[j]]])
            p2 = np.array([self.x[shoreline_loc[j]],
                           row[shoreline_loc[j]]])
            p3 = np.array([self.x[dune_crest_loc[j]:shoreline_loc[j]],
    ...
    def predict_shoreline(self, water_level=0, dune_crest='max', **kwargs):
        ...
        **kwargs : arguments
        ...
        sl_index = np.array([
            len(row) -
            np.argmin(np.flipud(row) <= water_level) -
            1 for row in self.z_interp])
        # Interp back to original x coordinates
